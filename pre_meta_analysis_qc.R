...
if(!is_ss_reanalysis) summary.dat[[sumstat_name]]=NA
...
summary.dat[summary.dat$Filter==paste0("EAF <",upper_EAF_bound,", >",lower_EAF_bound),paste0(sumstat_name,"_pct_of_prev_step")]=round(nrow(ss.dat)/summary.dat[summary.dat$Filter=="Initial # SNPs",sumstat_name]*100, digits=2)
...
summary.dat[summary.dat$Filter=="Complete cases",paste0(sumstat_name,"_pct_of_prev_step")]=round(nrow(ss.dat)/summary.dat[summary.dat$Filter==paste0("EAF <",upper_EAF_bound,", >",lower_EAF_bound),sumstat_name]*100, 2)
...
summary.dat[summary.dat$Filter=="Keep only A,C,T,G alleles",paste0(sumstat_name,"_pct_of_prev_step")]=round(nrow(ss.dat)/summary.dat[summary.dat$Filter=="Complete cases",sumstat_name]*100, 2)
...
summary.dat[summary.dat$Filter=="Remove non-numeric Betas",paste0(sumstat_name,"_pct_of_prev_step")]=round(nrow(ss.dat)/summary.dat[summary.dat$Filter=="Keep only A,C,T,G alleles",sumstat_name]*100, 2)
...
summary.dat[summary.dat$Filter=="Remove negative, non-numeric StErr",paste0(sumstat_name,"_pct_of_prev_step")]=round(nrow(ss.dat)/summary.dat[summary.dat$Filter=="Remove non-numeric Betas",sumstat_name]*100, 2)
...
if(add_imp_data_for_biovu){
	...
	summary.dat[summary.dat$Filter=="# BioVU snps after merging Rsq info",paste0(sumstat_name,"_pct_of_prev_step")]=round(nrow(ss.dat)/summary.dat[summary.dat$Filter=="Remove negative, non-numeric StErr",sumstat_name]*100, 2)
	rm(imp_data)
}
...
if(add_imp_data_for_biovu){
  summary.dat[summary.dat$Filter=="Rsq>0.3,INFO>0.8",paste0(sumstat_name,"_pct_of_prev_step")]=round(nrow(ss.dat)/summary.dat[summary.dat$Filter=="# BioVU snps after merging Rsq info",sumstat_name]*100, 2)
}else{
  summary.dat[summary.dat$Filter=="Rsq>0.3,INFO>0.8",paste0(sumstat_name,"_pct_of_prev_step")]=round(nrow(ss.dat)/summary.dat[summary.dat$Filter=="Remove negative, non-numeric StErr",sumstat_name]*100, 2)
}

ss.dat=ss.dat[ss.dat[,imp_column]<=1,]
summary.dat[summary.dat$Filter=="Rsq<1,INFO<1",sumstat_name]=nrow(ss.dat)
summary.dat[summary.dat$Filter=="Rsq<1,INFO<1",paste0(sumstat_name,"_pct_of_prev_step")]=round(nrow(ss.dat)/summary.dat[summary.dat$Filter=="Rsq>0.3,INFO>0.8",sumstat_name]*100, 2)


#Create ChrPosID column to merge files with (must ensure all sum stats are in the same build for this)
#remake ssfile to match xchr labels in ref and sumstats files
if(is_x_present & ref_x_name!=x_col){
  ss.dat[ss.dat[[chr_col]]==x_col,chr_col]=ref_x_name
}
...
summary.dat[summary.dat$Filter=="# SNPs after merging with Ref EAF file",paste0(sumstat_name,"_pct_of_prev_step")]=round(nrow(ref_ss_eaf.dat)/summary.dat[summary.dat$Filter=="Rsq<1,INFO<1",sumstat_name]*100, 2)
...
if(switch_ea_and_oa_cols==summary.dat[summary.dat$Filter=="# SNPs after merging with Ref EAF file",sumstat_name]){ #if all SNPs are misaligned
	...
	ref_ss_eaf.dat$ss_EAF=1-as.numeric(ref_ss_eaf.dat$ss_EAF)
...
summary.dat[summary.dat$Filter=="Final # SNPs where SS EA == Ref EA",paste0(sumstat_name,"_pct_of_prev_step")]=round(nrow(ref_ss_eaf.dat)/summary.dat[summary.dat$Filter=="# SNPs after merging with Ref EAF file",sumstat_name]*100, 2)


#if EAF is still actually MAF, then fix that
if(max(ref_ss_eaf.dat$ss_EAF)<=0.5){
...
#remove any SNPs that are outside of reference MAF range (must convert EAF to MAF for this)
...
ref_ss_eaf.dat=ref_ss_eaf.dat[(ref_ss_eaf.dat$ref_eaf_maf-(ref_maf_range/2))<=ref_ss_eaf.dat$ss_eaf_maf & ref_ss_eaf.dat$ss_eaf_maf<=(ref_ss_eaf.dat$ref_eaf_maf+(ref_maf_range/2)),]
summary.dat[summary.dat$Filter=="Exclude AEF outside 10% of ref AF",sumstat_name]=nrow(ref_ss_eaf.dat)
summary.dat[summary.dat$Filter=="Exclude AEF outside 10% of ref AF",paste0(sumstat_name,"_pct_of_prev_step")]=round(nrow(ref_ss_eaf.dat)/summary.dat[summary.dat$Filter=="Final # SNPs where SS EA == Ref EA",sumstat_name]*100, 2)
ref_ss_eaf.dat$ss_eaf_maf=NULL
ref_ss_eaf.dat$ref_eaf_maf=NULL
...
ref_ss_eaf.dat$C_const=1/sqrt(2*ref_ss_eaf.dat$ss_EAF*(1-as.numeric(ref_ss_eaf.dat$ss_EAF))) ##change EAF column to MAF
median_c_const=median(ref_ss_eaf.dat$C_const)
median_se=median(ref_ss_eaf.dat[[se_col]])
c_median_se=median_c_const/median_se
if(is_n_col){
	sqrt_max_n=sqrt(max(as.numeric(ref_ss_eaf.dat[[n_col]])))
}else{
...
if(is_first_sumstat_analysed){
	...
	c_constant.dat$C.median.se[c_constant.dat$Sumstat.name==sumstat_name]=round(c_median_se,4)
	c_constant.dat$sqrt.max.N[c_constant.dat$Sumstat.name==sumstat_name]=round(sqrt_max_n,4)
}else{
  c_constant.dat=read.table(paste0(output_dir,"c_div_by_median_se_vs_sqrt_n_across_biobanks.txt"), stringsAsFactors=FALSE, quote="",comment.char="",header=TRUE)
...
if(add_imp_data_for_biovu){
	...
	ref_ss_eaf.dat[c("ref_eaf","ref_ea","ref_oa","manual.p","C_const")]=NULL
	}
...
names(ref_ss_eaf.dat)[names(ref_ss_eaf.dat) == chr_col] <- "CHR"
names(ref_ss_eaf.dat)[names(ref_ss_eaf.dat) == bp_col] <- "BP"

if(is_n_col){
	names(ref_ss_eaf.dat)[names(ref_ss_eaf.dat) == n_col] <- "N"
}else{
  ref_ss_eaf.dat$N=sample_size
}
if(is_effect_BETA){
	names(ref_ss_eaf.dat)[names(ref_ss_eaf.dat) == effect_col] <- "BETA"
}else{
  ref_ss_eaf.dat$BETA=log(ref_ss_eaf.dat[[effect_col]])
  ref_ss_eaf.dat[[effect_col]] <- NULL
}
...
if(is_x_present){
	ss_auto.dat=ref_ss_eaf.dat[ref_ss_eaf.dat[[chr_col]]!=x_col & ref_ss_eaf.dat[[chr_col]]!=ref_x_name,]
	ss_x.dat=ref_ss_eaf.dat[ref_ss_eaf.dat[[chr_col]]==x_col | ref_ss_eaf.dat[[chr_col]]==ref_x_name,]
	ss_x.dat[[chr_col]]="23"
